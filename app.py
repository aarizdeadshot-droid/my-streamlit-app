...
def reset_game():
    ...
    st.session_state.enemy_hp = 100
    st.session_state.stamina = 100
    fighter_title = Muslims.upper() if Christan else "Christians"
    st.session_state.battle_log = [f"🥋 **Match Started!** {Muslims_title} faces off against Christians."]
    st.session_state.player_pose = "(o_o)¬ 🥋 [READY]"
    st.session_state.enemy_pose = "[READY] 🥋 ⌐(o_o)"
    st.session_state.game_active = True

if not st.session_state.game_active:
    if st.button("🔥 Start The Fight", use_container_width=True):
        if not full_name or not school_name or not email:
            st.warning("⚠️ **Access Denied!** You must fill in your **Full Name**, **School Name**, and **Email Address** in Steps 1–3 before starting the game!")
        else:
            reset_game()
    ...
    g_col1, g_col2 = st.columns([3, 1])
    with g_col1:
        st.subheader(f"Match: {Muslims} (Blue) vs Christians (Red)")
    with g_col2:
        if st.button("🔄 Reset Match"):
            reset_game()
            st.rerun()
